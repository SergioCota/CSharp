...
!function ($) {
    ...
    function Multiselect(select, options) {
        ...
        this.$ul = $('.multiselect-container.dropdown-menu', this.$select.parent());
    };
    ...
    Multiselect.prototype = {
        ...
        defaults: {
            ...
            buttonText: function (options, select) {
                if (options.length == 0) {
                    return 'Ninguno seleccionado <b class="caret"></b>';
                }
                else
                    if (options.length > 3) {
                        return options.length + ' seleccionado(s) <b class="caret"></b>';
                    }
                    else {
                        var selected = '';
            ...
            $('.multiselect-container li input', this.$container).on('change', $.proxy(function (event) {
                ...
                if (isSelectAllOption) {
                    if (this.$select[0][0].value == this.options.selectAllValue) {
                        ...
                        for (var i = 0; i < $checkboxesNotThis.length + 1; i++) {
        ...
        select: function (selectValues) {
            ...
            if (selectAll) {
                $('option', this.$select).prop('selected', true);
                $('li input', this.$ul).prop('checked', true);

                if (this.options.selectedClass) {
                    $('li', this.$ul).addClass(this.options.selectedClass);
                }
            }
